...
public class Desktop implements Item {
	...
	public void insertIntoDatabase(HttpServletRequest request) throws Exception {
		String preparedQuery = "INSERT INTO `info_desktops`(`id`, `image`, `price`, `stock`, `name`, `brand`, `processorBrand`, `processorType`, `processorCores`, `gpuBrand`, `gpuType`, `ramSize`, `hardDrive`, `ssd`, `promotion`, `warranty`, `color`, `screenSize`, `screenType`, `displayResolution`, `officeIncluded`, `operatingSystem`, `bundledSoftware`, `portsSlotsChassis`, `cameraMicrophone`, `clockSpeed`, `frontSideBus`, `maxExpandability`, `memorySlots`, `driveRotation`, `opticalDriveType`, `wifi`, `dlna`, `bluetooth`, `hdmi`, `usb`, `multicardReader`, `compatibleMemoryCards`, `width`, `depth`, `height`, `weight`, `more`) VALUES ((select ifnull(max(id), " + Constants.UNIQUE_IDENTIFIER_DESKTOP + "0000)+1 from info_desktops maxId),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
